...
export function PaystackPaymentModal({ payment, customerEmail, onClose, onPaymentOpened }: { payment: PaymentInitiationResult | null; customerEmail?: string | null; onClose: () => void; onPaymentOpened?: () => void; }) {
    ...
    try {
      if (gateway === 'flutterwave') {
        ...
        window.FlutterwaveCheckout?.({
          ...
          customer: { email },
          callback: () => { toast.success('Payment completed. Refreshing payment status may take a moment.'); onPaymentOpened?.(); onClose(); },
    ...
    <Modal open={Boolean(payment)} onClose={onClose} title="Complete payment" subtitle="Complete the transaction without leaving REPRONIG." size="md">
      {payment ? (
        <div className="space-y-5">
          ...
          {!['paystack', 'flutterwave'].includes(gateway) ? <Alert title="Gateway not embedded" description="Inline checkout is currently configured for Paystack and Flutterwave." /> : null}
          <div className="flex flex-wrap gap-3">
            <Button type="button" disabled={!['paystack', 'flutterwave'].includes(gateway) || opening} onClick={openCheckout}>{opening ? `Opening ${gatewayLabel}...` : `Pay with ${gatewayLabel}`}</Button>
