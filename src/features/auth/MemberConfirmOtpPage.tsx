...
import { getCurrentUser, resendMemberRegistrationOtp, verifyMemberRegistrationOtp } from '@/features/auth/api';
...
export function MemberConfirmOtpPage() {
  ...
  const verifyMutation = useMutation({
    mutationFn: verifyMemberRegistrationOtp,
  ...
  const resendMutation = useMutation({
    mutationFn: resendMemberRegistrationOtp,
  ...
  return (
    <AuthCard title="Confirm OTP" subtitle="Enter the 6-digit OTP sent to your email to complete member registration.">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-sm">
        ...
        <Link className="font-semibold text-[#AF1512]" to="/member/login">Back to login</Link>
      </div>
    </AuthCard>
  );
}
